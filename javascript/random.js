...
export function initRandom(data) {
	...
	elCount.addEventListener('keydown', e => { if (e.key === 'Enter') render(); });
	elCat.addEventListener('change', render);
	elTag.addEventListener('keydown', e => { if (e.key === 'Enter') render(); });
...
function render() {
	...
	if (catFilter) pool = pool.filter(bm => bm._category === catFilter);
	if (tagFilter) pool = pool.filter(bm =>
	...
	if (!picked.length) {
		...
		const parts = [];
		if (catFilter) parts.push(`in "${esc(catFilter)}"`);
		if (tagFilter) parts.push(`tagged "${esc(elTag.value.trim())}"`);
		elResults.innerHTML = `<div class="state-empty"><div class="state-icon">📭</div><p>No bookmarks ${parts.join(' ')}.</p></div>`;
		return;
	}

	elInfo.textContent = `🎲 ${picked.length} of ${pool.length} bookmark${pool.length === 1 ? '' : 's'}`;
...
function openAll() {
	lastPicked.forEach((bm, i) => {
		setTimeout(() => window.open(bm.url, '_blank'), i * 150);
	});
}
